...
def _read_process_memory(pid: int) -> dict[str, Any] | None:
    status = _read_kb_fields(Path(f"/proc/{pid}/status"), {"VmRSS", "VmHWM"})
    if not status:
        return None
    smaps = _read_kb_fields(Path(f"/proc/{pid}/smaps_rollup"), {"Pss"})
    return {
        "pid": pid,
...
def _read_node_memory() -> dict[str, Any] | None:
    ...
    return {
        ...
        "used_bytes": used,
        "usage_percent": round(used * 100 / total, 3),
    ...
    try:
        with output_file.open("a", encoding="utf-8", buffering=1) as output:
            while not stop_event.is_set():
                ...
                grouped = {
                    category: [
                        item for item in processes if item["category"] == category
                    ]
                    for category in (
                        "rollout_manager",
                        "controller",
                ...
                cluster_nodes = [
                    ...
                    for node, memory in zip(nodes, node_memories, strict=True)
                    if memory is not None
                ]
                resources = ray.cluster_resources()
                available = ray.available_resources()
                object_store_capacity = int(resources.get("object_store_memory", 0))
                object_store_available = int(
                    available.get("object_store_memory", 0)
                )
                record = {
                    ...
                    "master_node": next(
                        (node for node in cluster_nodes if node["role"] == "master"),
                        None,
                    ),
                    "cluster_nodes": cluster_nodes,
                    "proxy": _read_proxy_memory(Path(args.proxy_pid_file)),
